...
else
{
    ...
    $saldoVenta         = $rowVenta['totalVenta'] - $rowVenta['pagado'];
    ...
    foreach ($coleccionFactura as $rowFact)
    {
        ...
        if ($iva_F > 0 && $ieps_F == 0)
        {
            $importe            = round($subTotal_F / $factorIva, 2);
            $importeImpuestoIVA = round($subTotal_F - $importe, 2);
            $importeImpuestoIEPS= 0;
            $sumatoriaSinIva    += $importe;
            $tasaOCuotaIVATotal = $iva_F / 100;
            $valorUnitario      = round($precioU_F / $factorIva, 2);
        }
        elseif ($iva_F == 0 && $ieps_F > 0)
        {
            $importe            = round($subTotal_F / $factorIeps, 2);
            $importeImpuestoIVA = 0;
            $importeImpuestoIEPS= round($subTotal_F - $importe, 2);
            $sumatoriaSinIeps   += $importe;
            $tasaOCuotaIEPSTotal= $ieps_F / 100;
            $valorUnitario      = round($precioU_F / $factorIeps, 2);
        }
        elseif  ($iva_F == 0 && $ieps_F == 0)
        {
            $importe            = round($subTotal_F, 2);//round($subTotal_F, 2);
            $valorUnitario      = round($precioU_F, 2);
            $importeImpuestoIVA = 0;
            $importeImpuestoIEPS= 0;
            $sumatoriaSinIva    += $importe;
        }
        ...
        $xmlCFDI_c              .= 'ClaveUnidad="'.$claveUnidad.'" Cantidad="'.number_format($cantidad_F,3,".","").'" Unidad="'.$unidad.'" Descripcion="'.$descripcion.'" ValorUnitario="'.number_format($valorUnitario,2,".","").'" Importe="'.number_format($importe,2,".","").'" Descuento="0.00">';
    ...
    $xmlCFDI            = '<?xml version="1.0" encoding="utf-8"?><cfdi:Comprobante xmlns:cfdi="http://www.sat.gob.mx/cfd/3" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:tfd="http://www.sat.gob.mx/TimbreFiscalDigital" ';
