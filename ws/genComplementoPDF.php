...
{*/
    function genComplemento($idF,$mysqli,$vista)
    {
        ...
        $version            = $row['version'];
        $idFactura_relacion = $row['idRelacion'];
        $montoPago          = $row['montoPago'];
        $sql_sumaPagos      = "SELECT IFNULL(SUM(montoPago),0) AS sumaPagos FROM facturas WHERE idRelacion = $idFactura_relacion"; // Suma monto de los pagos realizados
        $result_sumaPagos   = $mysqli->query($sql_sumaPagos);
        $row_sumaPagos      = $result_sumaPagos->fetch_assoc();
        $montoPagado        = $row_sumaPagos['sumaPagos'];

        $sql_fact_rel       = "SELECT * FROM facturas WHERE id = $idFactura_relacion LIMIT 1";
        $result_fact_rel    = $mysqli->query($sql_fact_rel);
        $row_fact_rel       = $result_fact_rel->fetch_assoc();
        $total_fact_rel     = $row_fact_rel['total'];
        $saldoAnterior      = $total_fact_rel - $montoPagado;
        $saldoAnterior      = number_format($saldoAnterior,2,".","");
        $saldoInsoluto      = $total_fact_rel - $montoPagado;
        $saldoInsoluto      = number_format($saldoInsoluto,2,".","");
        //var_dump($row_fact_rel);
        ...
        $pdf->Cell(200,10,utf8_decode('COMPLEMENTO PARA RECEPCIÓN DE PAGOS'),$mB,1,'C');
        $pdf->SetFont('Arial','B',7.5);
        ...
        while ($row_d = $result_d->fetch_assoc())
        {
            ...
            $pdf->Cell(69,3.5,utf8_decode($row_d['descripcion']),$mB,0,'C');
            $pdf->Cell(23,3.5,utf8_decode(number_format(0,2,".",",")),$mB,0,'R');
            ...
            $pdf->Cell(24,3.5,utf8_decode(number_format(0,2,".",",")),$mB,1,'R');
        }
        ...
        $pdf->Cell(200,3.5,utf8_decode('COMPLEMENTO DE PAGOS v1.0'),$mB,1,'C');
        $pdf->SetFont('Arial','',7);
        $pdf->Cell(200,3.5,utf8_decode("VERSIÓN CFDI $version"),$mB,1,'C');
        $pdf->Cell(200,3.5,utf8_decode("USO: $usoCFDI"),$mB,1,'C');

        $pdf->SetFont('Arial','B',7.5);
        $pdf->Cell(55,3.5,utf8_decode('UUID RELACIÓN'),1,0,'L');
        $pdf->Cell(40,3.5,utf8_decode('ID FACT RELACIÓN'),1,0,'L');
        $pdf->Cell(30,3.5,utf8_decode('MONTO DEL PAGO'),1,0,'L');
        $pdf->Cell(30,3.5,utf8_decode('SALDO'),1,0,'L');
        $pdf->Cell(45,3.5,utf8_decode('FORMA PAGO'),1,1,'L');
        $pdf->SetFont('Arial','',7);
        $pdf->Cell(55,3.5,utf8_decode($row_fact_rel['folioFiscal']),$mB,0,'L');
        $pdf->Cell(40,3.5,utf8_decode($row_fact_rel['id']),$mB,0,'L');
        $pdf->Cell(40,3.5,utf8_decode("$".number_format($montoPago,2,".",",")),$mB,0,'L');
        $pdf->Cell(40,3.5,utf8_decode("$".number_format($saldoInsoluto,2,".",",")),$mB,0,'L');
        $pdf->Cell(40,3.5,utf8_decode($formaPago),$mB,0,'L');
        $pdf->Cell(200,4,utf8_decode(''),$mB,1,'R');
