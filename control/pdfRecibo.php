...
else
{
    ...
    if(isset($_GET['recibo']))
    {
        ...
        $sql = "SELECT
                    ...
                    pagosrecibidos.fechahora            AS fecha,
                    ...
                    pagosrecibidos.metodoPago           AS idFormaPago,
                    usuarios.nombre                     AS nombreUsuario,
                    usuarios.apellidop                  AS apellidopUsuario,
                    clientes.rsocial                    AS rSocial,
                    metodosdepago.nombre                AS nombreFormaPago,
                    (SELECT IFNULL(SUM(pagosrecibidos.monto),0)
                    FROM
                        pagosrecibidos
                    WHERE
                        pagosrecibidos.idventa = idVenta_)AS montoTotalRem,
                    (SELECT IFNULL(SUM(pagosrecibidos.monto),0)
                    FROM
                        pagosrecibidos
                    WHERE
                        pagosrecibidos.idFactura = idFactura_)AS montoTotalFac
                ...
                WHERE pagosrecibidos.id = $id
                LIMIT 1";
        $resultado = $mysqli->query($sql);
        if($resultado->num_rows == 1)
        {
            while($recibo = $resultado->fetch_assoc())
            {
                $mesTexto           = mes_a_texto($recibo['fecha']); //Fecha teórica de pago
                $fecha_dia          = date('d',strtotime($recibo['fecha']));
                $fecha_sql          = $recibo['fecha'];
                $parts              = explode('-',$recibo['fecha']);
                $listaVentas        = '';
                $contVentas         = 0;
                // $idVenta        = $recibo['idVenta'];
                // $sql            = "SELECT idventa, idFactura FROM pagosrecibidos WHERE fechahora = '$fecha_sql'";
                // $result_v       = $mysqli->query($sql);
                // while ($row_v= $result_v->fetch_assoc())
                // {
                if ($recibo['idFactura_'] != NULL)
                {
                    $total          = $recibo['montoTotalFac'];
                    $f              = $recibo['idFactura_'];
                    $sql            = "SELECT folioFiscal FROM facturas WHERE id = $f LIMIT 1";
                    $r_f            = $mysqli->query($sql);
                    $row_f          = $r_f->fetch_assoc();
                    $listaVentas    = 'Folio fiscal factura= "'.$row_f['folioFiscal'].'"';
                    //break;
                }
                else
                {
                    $esteIdRel      = $recibo['idPagoRelacion'];
                    ...
                    $total     = $row_estaSum['montoRem'];
                    $idPagoRelacionAnt = $recibo['idPagoRelacion'];
                }
                // }

                $reciboNo = str_pad($recibo['id'], 10, "0", STR_PAD_LEFT);
                $anio = $parts[0];
                ...
                $pdf->SetFont('Times','B',14);
                $pdf->Cell(37,10,'$'.number_format($total,2,".",","),0,0,'R',True);
                $pdf->Ln();
                $pdf->SetFont('Times','I',14);
                $pdf->Cell(40,10,utf8_decode('Recibí de:'),0,0,'R');
                $pdf->SetFont('Times','B',14);
                $pdf->Cell(125,10,utf8_decode($recibo['rSocial']),0,0,'C');
                $pdf->Ln();
                $pdf->SetFont('Times','I',14);
                $pdf->Cell(40,10,utf8_decode('La cantidad de:'),0,0,'R');
                $pdf->SetFont('Times','B',11);
                //$cantLetra = num2letras(3000.00);
                $pdf->Cell(145,10,numtoletras($total),0,0,'C',TRUE);
                $pdf->Ln();
                $pdf->SetFont('Times','I',14);
                $pdf->Cell(40,10,utf8_decode('Por concepto de:'),0,0,'R');
                $pdf->SetFont('Times','',14);
                $pdf->Cell(125,10,utf8_decode('Pago crédito de la(s) venta(s) No.:'),0,1,'C');
                $pdf->SetFont('Times','B',14);
                $pdf->Cell(200,10,utf8_decode($listaVentas),0,0,'C');
                //$pdf->Ln();
                $pdf->Cell(40,10);
                $pdf->SetFont('Times','',14);
                $pdf->Ln();
                $pdf->Cell(66,10);
                $pdf->SetFont('Times','I',14);
                $pdf->Cell(40,10,utf8_decode('Manzanillo, Col.'),0,0,'R');
                $pdf->Cell(10,10);
                $fechaD     = $recibo['fecha'];
                ...
                $pdf->SetXY(106,97);
                $pdf->SetFont('Times','I',14);
                $pdf->Cell(20,10,utf8_decode('Cajero:'),0,0,'R');
                $pdf->SetFont('Times','IB',14);
                $pdf->Cell(60,10,utf8_decode($recibo['nombreUsuario'].' '.$recibo['apellidopUsuario']),0,0,'C');
