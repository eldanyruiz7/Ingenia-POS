...
<?php include "nav.php" ?>
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-6 col-md-6 col-xs-6">
                    <h1 class="page-header"> <span id="spanCargando"><i class="fa fa-pencil-square-o" aria-hidden="true"></i> Editar Venta</span></h1>
                </div>
            ...
            <div class="row">
                <div class="col-lg-6 col-md-6">
                    <div class="form-group input-group input-group-lg">
                        <input id="inputBuscar" autofocus type="text" value="<?php echo (isset($_GET['idVenta'])) ? $_GET['idVenta'] : ''; ?>" class="form-control" placeholder="No. de ticket o remisión">
                        <span class="input-group-btn">
                            <button id="btnBuscar" class="btn btn-primary" type="button"><i class="fa fa-search"></i> Buscar
                            </button>
                        </span>
                    </div>
                </div>
    ...
    <script>
        ...
        function itemCompra(idSubVenta, codigo, idProducto, subTotal, precioU, cantidad, descripcion)
        {
        ...
        function preguntarAntesDeSalir ()
        {
            ...
            if ( ($(document).find("#btnGuardar").length < 1) == false || ($("#data").find("#idCliente").length < 1) == false)
            {
        ...
        function armarCotizacion()
        {
            ...
            $(".trItemLista").each(function(x)
            {
                ...
                prod                = new itemCompra(idSubVenta, codigo, idProducto, subTotal, precioU, cantidad, descripcion);
        ...
        function agregarProducto(e,peso)
        {
            ...
            if (existeProducto      == 0)
            {
                ...
                subTotal            = parseFloat(e.precioXunidad) * parseFloat(cantidadEnPeso);
                subTotal            = subTotal.toFixed(2);
                precioU             = parseFloat(e.precioXunidad);
                ...
                prod        = new itemCompra(-1,e.codigo, e.id, subTotal, precioU, cantidadEnPeso, e.descripcion);
                //itemCompra(idSubVenta, codigo, idProducto, subTotal, precioU, cantidad, descripcion, iva, ieps, unidadSat, claveSat);
        ...
        function actualizarNota()
        {
            ...
            for (var i = 0; i < listaProductos.length; i++)
            {
                precioU             = listaProductos[i]["precioU"];
                cantidad            = listaProductos[i]["cantidad"];
                nuevoSubTot         = redondearDec(parseFloat(precioU * cantidad));
                listaProductos[i]["subTotal"] = nuevoSubTot;
                nuevoTotal          = parseFloat(nuevoTotal) + parseFloat(nuevoSubTot);
                //console.log(listaProductos[i]["subTotal"]);
            }
            ...
            $(".trItemLista").each(function(x)
            {
                idSubVenta         = listaProductos[x]['idSubVenta'];
                ...
                //$(this).attr("name", idSubVenta);
                $(this).attr("name", x);
                $(this).attr("idSubVenta", idSubVenta);
        ...
        $(document).ready(function()
        {
            ...
            $("#btnBuscar").click(function()
            {
                ...
                $.ajax(
                {
                    method: "POST",
                    data: {id:id},
                    url: "../control/consultarVenta.php"
                }).done(function(d)
                {
                    if(d.status == 0)
                    {
            ...
            $(document).on("keydown",".inputPrecioU",function(e)
            {
                ...
                {
                    e.preventDefault();
                    $(this).parent().prev().find(".inputCantidad").focus();
                }
                if(e.keyCode == 38)
                {
                    e.preventDefault();
            ...
            $("body").on("change","#selectTipoDoc,#selectCondiciones,#selectCliente,#chkPU",function()
            {
                $("#btnGuardar").prop("disabled",false);
            });
            // $("body").on("change",".inputPrecioU",function()
                ...
                [
                    {
                        ...
                        click: function()
                        {
                            ...
                            var ocultarPU           = ($("#chkPU").prop("checked")) ? 1 : 0;
                            var remision            = $("#selectTipoDoc").val();
                            var esCredito           = $("#selectCondiciones").val();
                            $.ajax(
                            {
                                method: "POST",
                                url:"../control/actualizarVenta.php",
                                data: {listaProductos:listaProductosJSON,idVenta:idVenta,idCliente:idCliente,ocultarPU:ocultarPU,esCredito:esCredito,remision:remision}
                            }).done(function(p)
                            {
                                if(p.status == 1)
                                {
                                    ...
                                    $("#btnActualCot").html('Actualizar venta');
...
<div id="dialog-confirm-guardar" title="Actualizar venta">
    <h4>
